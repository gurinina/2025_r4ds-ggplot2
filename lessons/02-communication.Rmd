...
#|   engine size of cars is overlaid. 
...
1. Create a boxplot using the fuel economy data with x = `hwy` and y = `class` customized `title`, `subtitle`, `caption`, `x`, `y`, and `color` labels. +8pts
...
1. Use `geom_text()` with infinite positions to place text at the four corners of the plot. **Hint: construct a `data.frame` similar to `label_info` with the coordinate positions. +2pts
...
3. 
    c) How can you put a different label in each facet? +2pts
...
5. What are the four arguments to `arrow()`? +4pts
    How do they work?
...
The RColorBrewer scales are documented online at <https://colorbrewer2.org/> and made available in R via the **RColorBrewer** package, by Erich Neuwirth.
...
2. What is the first argument to every scale?
   How does it compare to `labs()`? +2pts
...
Setting ggplot2 theme globally in a session
...
1. What happens if you omit the parentheses in the following plot layout. Try it.
    Can you explain why this happens? +2pts
